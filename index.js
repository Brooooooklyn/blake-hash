...
switch (platform) {
  case 'android':
    switch (arch) {
      case 'arm64':
        ...
        break
      case 'arm':
        localFileExisted = existsSync(join(__dirname, 'blake.android-arm-eabi.node'))
        try {
          if (localFileExisted) {
            nativeBinding = require('./blake.android-arm-eabi.node')
          } else {
            nativeBinding = require('@napi-rs/blake-hash-android-arm-eabi')
          }
        } catch (e) {
          loadError = e
        }
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'blake.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./blake.darwin-universal.node')
      } else {
        nativeBinding = require('@napi-rs/blake-hash-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
